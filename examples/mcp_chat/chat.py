...
SYSTEM = (
    ...
    "- Recall matches facts by overlapping WORDS. Query with the concrete nouns used when a "
    "fact was stored ('editor', 'laptop', 'deploy region'), not abstract categories ('dev "
    "environment', 'my setup'). For a multi-field request, put all the specific nouns in one "
    "recall query (or call recall several times).\n"
    "- For a MULTI-HOP question ('the A of the B of X'), call recall_chain ONCE with start=X "
    "and the relations from X outward, e.g. recall_chain(start='Aurora', "
    "path=['owner','manager','timezone']). It resolves the whole chain in one step.\n"
