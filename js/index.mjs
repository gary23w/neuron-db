...
export class NeuronDB {
   ...
   * opts: { httpHandler?: (url: string) => Promise<string>, imports?: object }. httpHandler wires the
   * wasm's `host_http` (the `http` feature) so a FETCH route / the `fetch` op can reach the network.
   */
  static fromModule(mod, opts = {}) {
    const holder = {};
    const inst = new WebAssembly.Instance(mod, NeuronDB._imports(opts, holder));
    holder.ex = inst.exports;
    return new NeuronDB(inst.exports);
  }
  /** From wasm bytes (browser fetch, Node fs.readFile). Async — compiles then instantiates. */
  static async fromBytes(bytes, opts = {}) {
    const holder = {};
    const { instance } = await WebAssembly.instantiate(bytes, NeuronDB._imports(opts, holder));
    holder.ex = instance.exports;
  ...
  static async forBrowser(url, opts = {}) {
    return NeuronDB.fromBytes(await (await fetch(url)).arrayBuffer(), opts);
  }
  /** Build the wasm imports: a `host_http` shim (wired to opts.httpHandler, else a no-transport stub),
   *  merged with any caller-supplied imports. Harmless on builds without the `http` feature (ignored). */
  static _imports(opts, holder) {
    const env = { ...(opts.imports && opts.imports.env) };
    if (typeof env.host_http !== "function") {
      const handler = opts.httpHandler, td = new TextDecoder(), te = new TextEncoder();
      let tok = 0;
      env.host_http = (ptr, len) => {
        const ex = holder.ex;
        if (!handler || !ex) return -1; // no transport -> the wasm's fetch op reports unavailable
