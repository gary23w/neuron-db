"""The neuron: a tiny associative memory you write to in plain language and query by
cue. No model, no embeddings, no dependencies -- pure standard library.

  write   the most surprising content word of a statement becomes its value
  read    a question's words become a stemmed cue; the best-matching fact is found
          under relation-binding, and the value nearest the asked-about word is returned
  abstain no match -> "i don't know right now." (a memory that guesses is worse)

A neuron persists as just its raw facts (text + one flag); every index is recomputed
on load, so the recall logic can improve without migrating stored data, and a fact
costs ~30 bytes. This is the engine behind NeuronDB.
"""
...
def _words(s: str) -> set: return {_w1(w) for w in s.split()} - {""}
def _content(s: str) -> set: return _words(s) - STOP

def _stem1(w: str) -> str:
    w = IRR.get(w, w)
...
def _stem(ws) -> set: return {_stem1(w) for w in ws}

REL_S = _stem(REL); PETS = _stem(PETSRC); STOPVAL_S = _stem(STOPVAL)

def _isnum(w: str) -> bool:
    return any(c.isdigit() for c in w) or w.strip("?.!,'\"()").lower() in NUMWORDS

def _clip(s: str) -> str: return s.strip("?.!,;:'\"()[]{}")

def _surprise(w: str, i: int) -> float:
    s = 0.0; core = w.lower()
...
def _encode(text: str, entity: Optional[str]) -> Optional[dict]:
    u = text.strip()
    ...
    return {"t": text, "v": keep[0], "c": keep, "s": sorted(_stem(cont) | _stem(inject)),
            "h": (next(iter(_stem(inject))) if inject else head), "self": self_name and not inject}


def _pick_value(ep: dict, cue: set, want_num: bool):
...
class Neuron:
    """An associative memory. observe() to write, recall() to read."""
    def __init__(self, max_facts: int = 500):
        self.episodes: list[dict] = []
        self.last_entity: Optional[str] = None
        self.max_facts = max_facts

    def observe(self, text: str, entity: Optional[str] = None) -> list[dict]:
    ...
    # minimal persistence: only text + self flag; index rebuilt on load
    def dump(self) -> str:
        return json.dumps([[e["t"], 1 if e.get("self") else 0] for e in self.episodes], ensure_ascii=False)

    @classmethod
    def load(cls, blob: str, max_facts: int = 500) -> "Neuron":
        n = cls(max_facts)
    ...
    @property
    def fact_count(self) -> int: return len(self.episodes)
