    ...
    let (resp, returned) = match name.as_str() {
        ...
        "recall_associative" => {
            ...
            if q.is_empty() { (tool_err(id, "recall_associative needs a query"), 0) }
                ...
                // hops 0 (the default) = spread until it settles; a positive number bounds it
                let hops = json_num(body, "hops").unwrap_or(0).max(0) as usize;
